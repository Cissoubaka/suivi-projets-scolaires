"""Onglet de répartition des tâches (lecture seule) pour la visionneuse"""

from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QLabel, QComboBox,
    QScrollArea, QTableWidget, QTableWidgetItem, QCheckBox
)
...
class ViewerTaskAssignmentTab(ViewerTabBase):
    """Onglet de répartition des tâches (lecture seule pour élèves)"""
    ...
    def _apply_checkbox_style(self, checkbox, disabled=True):
        """Appliquer un style aux checkboxes (désactivées en viewer mode)"""
        checkbox.setMinimumWidth(24)
        checkbox.setMinimumHeight(24)
        checkbox.setEnabled(False)  # Toujours désactivé en mode viewer
    ...
    def create_widget(self):
        """Créer l'onglet de répartition des tâches en mode lecture seule"""
        widget = QWidget()
        layout = QVBoxLayout()
    ...
    def refresh_task_projects_combo(self):
        """Rafraîchir la liste des projets pour l'onglet répartition des tâches"""
    ...
    def refresh_task_assignments(self):
        """Afficher le tableau de répartition des tâches en lecture seule"""
        ...
        # Récupérer les catégories filtrées par niveau
        ...
        for row, (group_id, group_number, student_id, student_name) in enumerate(all_students):
            ...
            for col, category in enumerate(filtered_categories):
                ...
                # Récupérer l'assignation depuis la BD
                ...
                # Créer une checkbox en lecture seule
                checkbox = QCheckBox()
            ...
            # Colonne Total
            total = self._calculate_task_total(group_id, student_id)
    ...
    def _calculate_task_total(self, group_id, student_id):
        """Calculer le total des points d'assignation pour un élève"""
        total = 0
        
        if group_id in self.task_checkboxes and student_id in self.task_checkboxes[group_id]:
            for category in self.task_categories:
                ...
                if cat_id in self.task_checkboxes[group_id][student_id]:
                    ...
                    if checkbox.isChecked() and cat_points:
                        total += cat_points
